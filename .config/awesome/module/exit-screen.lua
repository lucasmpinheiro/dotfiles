...
local buildButton = function(icon, text)
...
local poweroff = buildButton(icons.power_off, "Shutdown")
poweroff:connect_signal("button::release", function()
  poweroff_command()
end)

local reboot = buildButton(icons.reboot, "Restart")
reboot:connect_signal("button::release", function()
  reboot_command()
end)

local suspend = buildButton(icons.suspend, "Sleep")
